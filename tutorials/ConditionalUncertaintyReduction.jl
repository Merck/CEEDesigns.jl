...
# The operational costs are defined as $400 and a 7-day turnaround for each in vitro assay and $4,000 and a 21-day turnaround 
# for the in vivo k_puu assay. 
...
# The MCTS-DPW solver is run with `n_iterations = 10,000` per MCTS run, to keep runtime manageable for the tutorial.
# `exploration_constant = 5.0` which controls the balance between exploration of new actions with exploitation of known high-value 
# actions within the search tree, and `depth = 5`, again keeping runtime down. Setting `depth = 11` would allow the planner to 
...
for (idx, evidence) in enumerate(state_init_list)
    println("\n", "="^60)
    println("  Processing $(scenarios[idx])")
    println("="^60)
...
for tau in taus
    ...
    summary_table = DataFrame(summary_data)
    CSV.write("summary_tau_$(Int(round(tau*100)))%.csv", summary_table)
